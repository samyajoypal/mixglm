...
expectation-maximization (EM) algorithm; lasso-selected supports are
subsequently scored by unpenalized active-set refits, and the number and
composition of components are searched over support-compatible family tuples.
For a fixed
...
develop a penalized generalized expectation-maximization algorithm, active-set
refit scoring, and a support-aware search over the number and composition of
components.  Third, for a fixed regular family
...
root mean squared error (RMSE), mean absolute error (MAE), integrated
completed likelihood (ICL), fitted mixture weights, and component-specific
active sets.  Penalized candidates are reported with their screening score and,
when available, their unpenalized active-set refit score.  We also flag selected
models containing an intercept-only component, because such a component may
reflect either genuine residual heterogeneity or excessive shrinkage under a
common penalty.  The RAND analysis additionally includes a participant
bootstrap because it is the stronger independent count example and has a final
fixed family pair validated across splits.
...
models.  Dark cells indicate selected non-intercept covariates.  The figure is
used to assess whether the selected family structure is accompanied by
meaningful component-specific covariate structure.}
...
effective-sample-size-adjusted tuning is a natural extension.  We therefore
report whether any selected component is intercept-only and treat such cases as
diagnostic rather than definitive evidence that all measured covariates are
irrelevant in that component.
